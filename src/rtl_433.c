...
#undef DECL
    ...
    if (!in_filename) {
#ifdef SOAPYSDR
        r = verbose_device_search(dev_query, &dev, &stream, SOAPY_SDR_CS8); // or SOAPY_SDR_CS16; SOAPY_SDR_CU8 does not work
...
//    uint16_t *buffer = malloc(out_block_elems * SoapySDR_formatToSize(SOAPY_SDR_CS16));
    uint8_t *buf8 = malloc(out_block_elems * SoapySDR_formatToSize(SOAPY_SDR_CU8));
#endif
        while (!do_exit) {
#ifdef SOAPYSDR
            ...
            do {
                ...
                do {
                    total_read += r;
                    buffs[0] = &buf8[total_read * 2];
                    r = SoapySDRDevice_readStream(dev, stream, buffs, out_block_elems - total_read, &flags, &timeNs, timeoutNs);
                    //fprintf(stderr, "readStream ret=%d, flags=%d, timeNs=%lld\n", r, flags, timeNs);
                } while (r >= 0 && total_read < out_block_elems);
                //fprintf(stderr, "readStream ret=%d (%d), flags=%d, timeNs=%lld\n", total_read, out_block_size, flags, timeNs);
                if (r >= 0)
                {
                ...
                // if converting CS16 to CU8
                //for (i = 0; i < n_read; ++i)
                //    buf8[i] = ((int16_t)buffer[i] / 32767.0 * 128.0 + 127.4);
                // if converting CS8 to CU8
                //for (i = 0; i < n_read; ++i)
                //    buf8[i] = (int8_t)buf8[i] + 128;
